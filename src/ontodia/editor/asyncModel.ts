...
import { СlassTreeProvider } from '../diagram/classTreeProvider';
...
export class AsyncModel extends DiagramModel {
    ...
    private treeClasses = new СlassTreeProvider();
    ...
    get getClasses() {
        return this.treeClasses.classes;
    }
    ...
    private setClassTree(rootClasses: ClassModel[]) {
        const addClass = (base: FatClassModel | undefined, classModel: ClassModel) => {
            ...
            if (!Boolean(this.treeClasses.getClass(richClass.id)) || !this.treeClasses.cycle(base, id)) {
                richClass.setBase(base);
                this.graph.addClass(richClass);
                this.treeClasses.addClass(richClass);
